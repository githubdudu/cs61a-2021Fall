...
def balanced(m):
    ...
    True
    """
    left_ba, right_ba = True, True

    if is_planet(end(left(m))):
        torque_left = length(left(m)) * size(end(left(m)))
    else:
        torque_left = length(left(m)) * total_weight(end(left(m)))
        left_ba = balanced(end(left(m)))

    if is_planet(end(right(m))):
        torque_right = length(right(m)) * size(end(right(m)))
    else:
        torque_right = length(right(m)) * total_weight(end(right(m)))
        right_ba = balanced(end(right(m)))

    return left_ba and right_ba and torque_left == torque_right
