...
def roll_dice(num_rolls, dice=six_sided):
    ...
    >>> roll_dice(1, make_test_dice(1,2,3))
    1
    >>> roll_dice(2, make_test_dice(1,2,3))
    1
    >>> roll_dice(3, make_test_dice(1,2,3))
    1
    >>> roll_dice(1, make_test_dice(3,2,3))
    3
    >>> roll_dice(2, make_test_dice(3,2,3))
    5
    >>> roll_dice(1, make_test_dice(3,2,3))
    8
    ...
    for i in range(num_rolls):
        ...
        if (outcome == 1):
            sow_sad_mark = True
    return 1 if sow_sad_mark else outcomes
    ...
    def say(score0, score1):
        if(who == 0):
            ...
            if(highest > running_high):
                print(highest, "point(s)! That's a record gain for Player 0!")
            return announce_highest(who, score0, max(highest, running_high))
        else:
            highest = score1 - last_score 
            if(highest > running_high):
                print(highest, "point(s)! That's a record gain for Player 1!")
            return announce_highest(who, score1, max(highest, running_high))
    return say
    # END PROBLEM 7
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    strategy = picky_piggy_strategy(score, opponent_score, cutoff, num_rolls)
    if(strategy == 0):
        return strategy
    elif(hog_pile(score + picky_piggy(opponent_score), opponent_score) >= cutoff):
        return 0
    return num_rolls
    # END PROBLEM 11
