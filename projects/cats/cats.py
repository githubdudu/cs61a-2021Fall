...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def about_topic(one_sentence):
        splited_sentence = split(lower(remove_punctuation(one_sentence)))
...
def accuracy(typed, reference):
    ...
    if typed_len == 0 and ref_len == 0:
        return 100.0
    elif typed_len == 0:
        return 0.0

    for i in range(min(typed_len, ref_len)):
...
def wpm(typed, elapsed):
    ...
    return len(typed) / TYPICAL_WORD_LEN / (elapsed / SECONDS_IN_ONE_MINUTE)
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    words_with_diff = [[word, diff_function(typed_word, word, limit)] for word in valid_words]
    word_with_min_diff = min(words_with_diff, key=lambda pairs: pairs[1])
    if word_with_min_diff[1] > limit:
        return typed_word
    elif typed_word in valid_words:
        return typed_word

    return word_with_min_diff[0]
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0: # base condition
        ...
        # end of one of the string 
        # BEGIN
        return abs(len(start) - len(goal))
        # END

    elif start[0] == goal[0]:  # Feel free to remove or add additional cases
        # compare first letter, then use recursion
        # BEGIN
        return minimum_mewtations(start[1:], goal[1:], limit)
        # END
    
    else:
        # classical tree recursion, use min to aggregate
        add = goal[0] + start  # Fill in these lines
        remove = start[1:]
        substitute = goal[0] + start[1:]
        # BEGIN
        return 1 + min(minimum_mewtations(add, goal, limit - 1),\
            minimum_mewtations(remove, goal, limit - 1),\
            minimum_mewtations(substitute, goal, limit - 1),)
        # END
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[one_player_timestamp[i + 1] - one_player_timestamp[i] for i in range(len(one_player_timestamp) - 1)] for one_player_timestamp in times_per_player]
...
def fastest_words(match):
    ...
    for w_num in word_indices:
        min_time = time(match, 0, w_num)
        index = 0
        for p_num in player_indices:
            if min_time > time(match, p_num, w_num):
                min_time = time(match, p_num, w_num)
                index = p_num
        result[index] += [word_at(match, w_num)]
    return result
    # END PROBLEM 10
