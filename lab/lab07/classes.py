...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        # add stats
        for card_in_deck in player.deck.cards:
            card_in_deck.attack += opponent_card.attack
            card_in_deck.defense += opponent_card.defense
        # clear opponent's deck
        for card_in_op in opponent.deck.cards[:]:
            if card_in_op.attack == opponent_card.attack and card_in_op.defense == opponent_card.defense:
                opponent.deck.cards.remove(card_in_op)
