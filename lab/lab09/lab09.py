...
def non_decrease_subseqs(s):
    ...
    #     return [[], s]
    # elif s[0] > s[1]:
    #     return [[s[0]]] + non_decrease_subseqs(s[1:])
    # else:
...
class Account:
    ...
    def __repr__(self):
        depos_count = sum([1 for trans in self.transactions if trans[0] == 'deposit'])
        withdraw_count = sum([1 for trans in self.transactions if trans[0] == 'withdraw'])
        return f'Accountholder: {self.holder}, Deposits: {depos_count}, Withdraws: {withdraw_count}'
...
def insert(link, value, index):
    ...
    temp = Link(pointer.first, pointer.rest)
    pointer.first = value
    pointer.rest = temp
...
def long_paths(t, n):
    ...
    if t.is_leaf():
        ...
        rest = sum([long_paths(branch, n - 1) for branch in t.branches], [])
        return insert_into_all(t.label, rest)
